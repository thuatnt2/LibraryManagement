...
class User extends UserMgmtAppModel {
	...
	var $hasOne = array('Reader' => array('className' => 'Usermgmt.Reader', 'foreignKey' => 'user_id'));
	...
	public function validateReader() {
		...
		$validate1 = array(
			'username' => array(
				...
				'mustUnique' => array(
					'rule' => 'isUnique',
					'message' => 'Mã số này đã tồn tại',
					'last' => true),
				'mustBeLonger' => array(
					'rule' => array('minLength', 6),
					'message' => 'Mã số bạn đọc phải ít nhất 6 kí tự',
					'last' => true),
			),
			'fullname' => array(
				'mustNotEmpty' => array(
					'rule' => 'notEmpty',
					'message' => 'Nhập họ tên bạn đọc')
			),
			'is_teacher' => array(
				'mustNotEmpty' => array(
					'rule' => 'notEmpty',
