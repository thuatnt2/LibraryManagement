		...
		foreach ($data_provider as $data) {
				...
				if ($column['column'] == 'is_active') {
					if ($data[$column['Model']]['is_active'] == 1) {
						...
						$output.= $this->Html->link(
								$this->Html->image('/images/approve.png'), array('controller' => 'app', 'action' => 'active', $this->_controller, $column['Model'], $data[$column['Model']]['id'], 0, $this->_page), array('escape' => false));
					} else {
						$output.= $this->Html->link(
								$this->Html->image('/images/dis-approve.png'), array('controller' => 'app', 'action' => 'active', $this->_controller, $column['Model'], $data[$column['Model']]['id'], 1, $this->_page), array('escape' => false));
					}
				} else {
			...
			if ($this->_options['actions_column']) {
				...
				if ($this->_options['edit_action']) {
					$output.= $this->Html->link(
							$this->Html->image('/images/edit.png'), array('controller' => $this->_controller, 'action' => 'edit', $data[$column['Model']]['id']), array('escape' => false)
					);
				}
				if ($this->_options['delete_action']) {
					$output.= $this->Form->postLink(
							$this->Html->image('/images/delete.png'), array('controller' => $this->_controller, 'action' => 'delete', $data[$column['Model']]['id']), array('escape' => false), 'Bạn có chắc chắn muốn xóa ?'
					);
				}
				$output.= '</td>';
			}
